...
while i <= 10
    i
    [temp_results,temp_target,temp_output,temp_number] = cascade(number,P,T,0.8);
    if i <= 6
        if temp_results(1,1) < 0.82
           continue
        end
    else
        if temp_results(1,1) < 0.8 || temp_results(1,1) > 0.82
           continue
        end
    end

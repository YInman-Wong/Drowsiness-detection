...
while i <= 10
    i
    [temp_results,temp_target,temp_output,temp_number] = knn(number,P,T,0.1);
    if i <= 2
        if temp_results(1,1) < 0.84
           continue
        end
    else
...
saveas(b,'.\4-模型搭建\knn_results\individual.tiff')

clearvars -except results target output participant h individual_analysis_ip b
save('.\4-模型搭建\knn_results\knn_results.mat')
...
while i <= 10
    i
    [temp_results,temp_target,temp_output,temp_number] = feed(number,P,T,0.8);
    if i <= 6
...
saveas(b,'.\4-模型搭建\feed_results\individual.tiff')

clearvars -except results target output participant h individual_analysis_ip b
save('.\4-模型搭建\feed_results\feed_results.mat')